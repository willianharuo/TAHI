...
command test_TEST-TAR-HOST TEST-TAR-HOST {
    ...
    print "BASENAME> Configure YOUR DEVICE to have the same LLA as DUP_ADDR.";
    ...
    # Step 2 reverse
    print "";
    print "BASENAME> Step 34 (Spec)";
    print "BASENAME> TEST-TAR-HOST enables interface on Network1 with the same .";
...
command test_REF1 REF1 {
    ...
    print "BASENAME> REF1 transmits ICMPv6 Echo Request to DUP_ADDR.";
    print "";
    execute "/bin/sh -c ping6 -c 5 -I REF1_IF1 DUP_ADDR | tee /tmp/1.2.D.REF.TEST-TAR-HOST_NAME.result";
    delay 2;
...
command setup_DUMPER DUMPER {
    delay 5;
    sync start_setup;
    execute "tcpdump -i DUMPER_IF1 -c 10000 -s 0 -w /tmp/1.2.D.LOGO_NAME.TEST-TAR-HOST_NAME.Network1.dump";
}
