...
command test_TEST-TAR-HOST TEST-TAR-HOST {
    ...
    print "BASENAME> Transmit ICMPv6 Echo Request from YOUR DEVICE to TEST-TAR-HOST";
    print "BASENAME> Link-Local Address TEST-TAR-HOST_IF1_LLA and Save the command ";
    print "BASENAME> result log as /tmp/1.1.D.LOGO_NAME.TEST-TAR-HOST_NAME.result.";
    print "BASENAME> Press Enter key for continue.";
    ...
    execute "/bin/sh -c ping6 -c 5 -I TEST-TAR-HOST_IF1 LOGO_IF1_LLA | tee /root/1.1.D.TEST-TAR-HOST_NAME.LOGO_NAME.result";
...
command setup_DUMPER DUMPER {
    delay 5;
    sync start_setup;
    execute "tcpdump -i DUMPER_IF1 -c 10000 -s 0 -w /tmp/1.1.D.LOGO_NAME.TEST-TAR-HOST_NAME.Network1.dump";
}
