...
syncevent sync_step3 test_TEST-TAR-HOST, test_REF2;
syncevent sync_step4 test_TEST-TAR-HOST, test_REF2;
syncevent sync_step5 test_TEST-TAR-HOST, test_REF2;

# waitevent
waitevent wait_step4 test_TEST-TAR-HOST;
...
command test_TEST-TAR-HOST TEST-TAR-HOST {
    ...
    print "BASENAME> Router Lifetime set to 600 seconds and ";
    print "BASENAME> Router Advertisement Intervals set to approximately 60 seconds";
    print "BASENAME> on both Network 1 and Network 2.";
...
command test_REF2 REF2 {
    ...
    execute "/bin/sh -c ping6 -c 5 TEST-TAR-HOST_IF1_PREFIX1_GA | tee /tmp/1.4.A.REF.TEST-TAR-HOST_NAME.result";
    delay 1;
...
command setup_DUMPER DUMPER {
    delay 5;
    sync start_setup;
    execute "/bin/sh -c /usr/sbin/tcpdump -i DUMPER_IF1 -c 10000 -s 0 -w /tmp/1.4.A.LOGO_NAME.TEST-TAR-HOST_NAME.Network1.dump&";
    execute "/bin/sh -c /usr/sbin/tcpdump -i DUMPER_IF2 -c 10000 -s 0 -w /tmp/1.4.A.LOGO_NAME.TEST-TAR-HOST_NAME.Network2.dump&";
}
