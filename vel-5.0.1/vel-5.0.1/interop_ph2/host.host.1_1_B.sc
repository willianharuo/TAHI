...
command test_TEST-TAR-HOST TEST-TAR-HOST {
    ...
    print "BASENAME> Global Address TEST-TAR-HOST_IF1_PREFIX1_GA and Save the command ";
    print "BASENAME> result log as /tmp/1.1.B.LOGO_NAME.TEST-TAR-HOST_NAME.result.";
    print "BASENAME> Press Enter key for continue.";
    ...
    execute "/bin/sh -c ping6 -c 5 LOGO_IF1_PREFIX1_GA | tee /root/1.1.B.TEST-TAR-HOST_NAME.LOGO_NAME.result";
...
command setup_DUMPER DUMPER {
    delay 5;
    sync start_setup;
    execute "tcpdump -i DUMPER_IF1 -c 10000 -s 0 -w /tmp/1.1.B.LOGO_NAME.TEST-TAR-HOST_NAME.Network1.dump";
}
