...
command test_TEST-TAR-ROUTER TEST-TAR-ROUTER {
    ...
    print "BASENAME> result log as /tmp/1.1.D.LOGO_NAME.TEST-TAR-ROUTER_NAME.result.";
    print "BASENAME> Press Enter key for continue.";
    ...
    execute "/bin/sh -c ping6 -c 5 -I TEST-TAR-ROUTER_IF1 LOGO_IF1_LLA | tee /root/1.1.D.TEST-TAR-ROUTER_NAME.LOGO_NAME.result";
...
command setup_DUMPER DUMPER {
    delay 5;
    sync start_setup;
    execute "tcpdump -i DUMPER_IF1 -c 10000 -s 0 -w /tmp/1.1.D.LOGO_NAME.TEST-TAR-ROUTER_NAME.Network1.dump";
}
