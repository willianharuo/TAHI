...
command test_TEST-TAR-ROUTER TEST-TAR-ROUTER {
    ...
    # Step 2 reverse
    print "";
    print "BASENAME> Step 34 and 35 (Spec)";
    print "BASENAME> Configure YOUR DEVICE interface on Network1 to have the same";
    #
    # not use a link-local address formed from EUI-64
...
command test_REF1 REF1 {
    ...
    #
    # not use a link-local address formed from EUI-64
    # execute "/bin/sh -c ping6 -c 5 -I REF1_IF1 TEST-TAR-ROUTER_IF1_LLA | tee /tmp/1.2.D.REF.LOGO_NAME.result";
    execute "/bin/sh -c ping6 -c 5 -I REF1_IF1 DUP_ADDR | tee /tmp/1.2.D.REF.LOGO_NAME.result";
    delay 2;
    ...
    #
    # not use a link-local address formed from EUI-64
    # execute "/bin/sh -c ping6 -c 5 -I REF1_IF1 LOGO_IF1_LLA | tee /tmp/1.2.D.REF.TEST-TAR-ROUTER_NAME.result";
    execute "/bin/sh -c ping6 -c 5 -I REF1_IF1 DUP_ADDR | tee /tmp/1.2.D.REF.TEST-TAR-ROUTER_NAME.result";
    delay 2;
...
command setup_DUMPER DUMPER {
    delay 5;
    sync start_setup;
    execute "tcpdump -i DUMPER_IF1 -c 10000 -s 0 -w /tmp/1.2.D.LOGO_NAME.TEST-TAR-ROUTER_NAME.Network1.dump";
}
