...
command test_TEST-TAR-ROUTER TEST-TAR-ROUTER {
    ...
    print "BASENAME> Bring up the interface on TAR-Host1 that is connected to Network1.";
...
command test_REF1 REF1 {
    ...
    execute "/bin/sh -c ping6 -c 5 LOGO_IF1_PREFIX1_GA | tee /tmp/1.3.A.REF.LOGO_NAME.result";
...
command setup_DUMPER DUMPER {
    delay 5;
    sync start_setup;
    execute "tcpdump -i DUMPER_IF1 -c 10000 -s 0 -w /tmp/1.3.A.LOGO_NAME.TEST-TAR-ROUTER_NAME.Network1.dump";
}
