...
command test_TEST-TAR-ROUTER TEST-TAR-ROUTER {
    ...
    print "BASENAME> result log as /tmp/1.1.I.LOGO_NAME.TEST-TAR-ROUTER_NAME.result.";
    print "BASENAME> Press Enter key for continue.";
    ...
    execute "/bin/sh -c ping6 -c 5 -I TEST-TAR-ROUTER_IF1 ff02::1 | tee /root/1.1.I.TEST-TAR-ROUTER_NAME.LOGO_NAME.result";

    delay 1;

    print "BASENMAE> Step 39 1/2 (Spec)";
    print "BASENAME> Transmit ICMPv6 Echo Request from YOUR DEVICE to";
    print "BASENAME> the All Routers multicast address (FF02::2) and Save ";
    print "BASENAME> the command result log as /tmp/1.1.F.LOGO_NAME.TEST-TAR-ROUTER_NAME.result.";
    print "BASENAME> Press Enter key for continue.";
    wait wait_step2;

    print "BASENMAE> Step 39 2/2 (Spec)";
    print "BASENAME> TEST-TAR-ROUTER transmits ICMPv6 Echo Request to the All Routers ";
    ...
    TEST-TAR-ROUTER_CMD_CLEAR_NCE_IF3;
    TEST-TAR-ROUTER_CMD_PRINT_NCE_IF3;
    execute "/bin/sh -c ping6 -c 5 -I TEST-TAR-ROUTER_IF1 ff02::2 | tee -a /root/1.1.I.TEST-TAR-ROUTER_NAME.LOGO_NAME.result";
...
command setup_DUMPER DUMPER {
    delay 5;
    sync start_setup;
    execute "tcpdump -i DUMPER_IF1 -c 10000 -s 0 -w /tmp/1.1.I.LOGO_NAME.TEST-TAR-ROUTER_NAME.Network1.dump";
}
