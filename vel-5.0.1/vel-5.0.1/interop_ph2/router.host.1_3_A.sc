...
command test_REF1 REF1 {
    ...
    execute "/bin/sh -c ping6 -c 5 TEST-TAR-HOST_IF1_PREFIX1_GA | tee /tmp/1.3.A.REF.TEST-TAR-HOST_NAME.result";
...
command setup_DUMPER DUMPER {
    delay 5;
    sync start_setup;
    execute "tcpdump -i DUMPER_IF1 -c 10000 -s 0 -w /tmp/1.3.A.LOGO_NAME.TEST-TAR-HOST_NAME.Network1.dump";
}
